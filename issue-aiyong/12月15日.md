...
一个简单的计数器组件就完成了，而在函数组件中，由于没有 this 这个黑魔法，React hooks 通过 useState 来帮我们保存组件的状态。
...
- 2.而第二个 useEffect 中传递了一个空数组[]，这种情况下只有在组件初始化或销毁的时候才会触发，用来代替 componentDidMount 和 componentWillUnmount，慎用；
  ...
  - 下面的 useEffect 用法实现了类组件中的 `componentDidMounted()` 和 `componentWillUnmount`
...
#### 在 useEffect 中发送 ajax 请求数据

```jsx
//-------直接发送请求----------------
useEffect(() => {
    fetch('http://example.com/api/demo')
...
//-------调用ajax方法---------------
function loadData() {
    return fetch('http://example.com/api/demo')
    	.then(response => response.json())
    	.then(result => {
        	return result;
    	})
...
> 因为 loadData() 函数中返回的 fetch 请求中 返回了 result。所以 useEffect 可能会将该返回值当成 return 的函数，因此会报错。
